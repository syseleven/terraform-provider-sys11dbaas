...
1. Import the database as described above.
1. Remove existing V1 from state by `terraform state rm sys11dbaas_database.default`. Be careful this step is invasive.
1. Run `terraform plan` to verify that no change is required.
